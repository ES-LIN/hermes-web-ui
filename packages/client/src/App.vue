...
const useAdminLayout = computed(() => Boolean(route.meta.adminLayout))
    ...
    <NMessageProvider>
      ...
      <NDialogProvider>
        <NNotificationProvider>
          ...
          <div v-if="ready" class="app-layout" :class="{ 'no-sidebar': !useAdminLayout }">
            <button v-if="useAdminLayout" class="hamburger-btn" @click="appStore.toggleSidebar">
              <img src="/logo.png" alt="Menu" style="width: 24px; height: 24px;" />
            </button>
            <div v-if="useAdminLayout && appStore.sidebarOpen" class="mobile-backdrop" @click="appStore.closeSidebar" />
            <AppSidebar v-if="useAdminLayout" />
            <main class="app-main">
              <router-view />
            </main>
          </div>
          <SessionSearchModal v-if="useAdminLayout" />

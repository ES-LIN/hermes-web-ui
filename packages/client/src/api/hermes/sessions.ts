...
export async function clearAdminTestSessions(profile?: string | null): Promise<{ success: boolean; deleted: number }> {
  const params = new URLSearchParams()
  if (profile) params.set('profile', profile)
  const query = params.toString()
  return request<{ success: boolean; deleted: number }>(
    `/api/hermes/sessions/admin-test${query ? `?${query}` : ''}`,
    { method: 'DELETE' },
  )
}

export async function promoteAdminTestSession(id: string, profile?: string | null): Promise<{ success: boolean; session: SessionDetail }> {
  const params = new URLSearchParams()
  if (profile) params.set('profile', profile)
  const query = params.toString()
  return request<{ success: boolean; session: SessionDetail }>(
    `/api/hermes/sessions/admin-test/${encodeURIComponent(id)}/promote${query ? `?${query}` : ''}`,
    { method: 'POST' },
  )
}

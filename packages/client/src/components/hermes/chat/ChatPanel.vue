...
import { clearAdminTestSessions, promoteAdminTestSession, renameSession, setSessionWorkspace, batchDeleteSessions, exportSession } from "@/api/hermes/sessions";
...
function sessionHref(sessionId: string) {
  return router.resolve({
    name: props.adminTest ? "admin.session" : "chat.session",
    params: { sessionId },
  }).href;
}
...
async function handleProfileFilterChange(value: string) {
  ...
  await chatStore.loadSessions(chatStore.sessionProfileFilter, null, props.adminTest ? 'admin_test' : undefined);
}
...
async function confirmNewChat() {
  const session = chatStore.newChat({
    ...
    source: props.adminTest ? 'admin_test' : undefined,
  });
  await router.push({
    name: props.adminTest ? "admin.session" : "chat.session",
...
async function handleClearAdminTests() {
  try {
    const result = await clearAdminTestSessions(chatStore.sessionProfileFilter)
    await chatStore.loadSessions(chatStore.sessionProfileFilter, null, 'admin_test')
    if (!chatStore.activeSession) {
      const session = chatStore.newChat({ source: 'admin_test' })
      await router.replace({ name: 'admin.session', params: { sessionId: session.id } })
    }
    message.success(t('chat.adminTestCleared', { count: result.deleted }))
  } catch (err: any) {
    message.error(err?.message || t('chat.adminTestClearFailed'))
  }
}

async function handlePromoteAdminTestSession() {
  const sid = chatStore.activeSessionId
  if (!sid) return
  try {
    const result = await promoteAdminTestSession(sid, chatStore.activeSession?.profile)
    message.success(t('chat.adminTestPromoted'))
    if (result.session?.id) {
      await router.push({ name: 'chat.session', params: { sessionId: result.session.id } })
    }
  } catch (err: any) {
    message.error(err?.message || t('chat.adminTestPromoteFailed'))
  }
}
...
<template>
  <div class="chat-panel" :class="{ 'chat-panel--simple': props.simple }">
    <div
      ...
      <header class="chat-header">
        ...
        <div class="header-actions">
          ...
          <template v-if="currentMode === 'chat'">
            ...
            <NButton
              v-if="props.adminTest"
              size="small"
              secondary
              :disabled="!chatStore.activeSessionId"
              @click="handlePromoteAdminTestSession"
            >
              {{ t("chat.adminTestPromote") }}
            </NButton>
            <NPopconfirm
              v-if="props.adminTest"
              @positive-click="handleClearAdminTests"
            >
              <template #trigger>
                <NButton size="small" tertiary type="warning">
                  {{ t("chat.adminTestClear") }}
                </NButton>
              </template>
              {{ t("chat.adminTestClearConfirm") }}
            </NPopconfirm>
          </template>
        </div>
      </header>
    ...
    <div v-if="!props.simple" class="drawer-button-wrapper">

...
function handleSpeechToggle() {
  ...
  if (voiceSettings.provider.value === 'openai') {
    ...
    speech.openaiToggle(props.message.id, content, {
      ...
      streamingEnabled: voiceSettings.ttsStreamingEnabled.value,
    })
    return
  }
  ...
  if (voiceSettings.provider.value === 'mimo') {
    ...
    if (voiceSettings.mimoModel.value === 'mimo-v2.5-tts-voiceclone' && !voiceSettings.mimoCloneAudioDataUrl.value) {
    ...
    speech.mimoToggle(props.message.id, content, {
      ...
      cloneAudioDataUrl: voiceSettings.mimoCloneAudioDataUrl.value || undefined,
  ...
  autoPlayHandler = (e: Event) => {
    ...
    if (customEvent.detail.messageId === props.message.id && canPlaySpeech.value) {
      ...
      if (voiceSettings.provider.value === 'openai') {
        ...
        const apiKey = voiceSettings.mimoApiKey.value
        const hasCloneAudio = voiceSettings.mimoModel.value !== 'mimo-v2.5-tts-voiceclone' || !!voiceSettings.mimoCloneAudioDataUrl.value
        if (apiKey && hasCloneAudio) {

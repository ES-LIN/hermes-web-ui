...
import { NSelect, NInput, NButton, NSlider, NUpload, NSwitch, useMessage } from 'naive-ui'
...
async function beforeMimoCloneAudioUpload({ file }: { file: UploadFileInfo }) {
  const rawFile = file.file
  if (!rawFile) return false
  const fileName = rawFile.name.toLowerCase()
  const isSupportedAudio = ['audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/x-wav', 'audio/wave'].includes(rawFile.type)
    || fileName.endsWith('.mp3')
    || fileName.endsWith('.wav')
  if (!isSupportedAudio) {
    message.error(t('settings.voice.mimoCloneAudioTypeError'))
    return false
  }
  if (rawFile.size > 7.5 * 1024 * 1024) {
    message.error(t('settings.voice.mimoCloneAudioSizeError'))
    return false
  }
  try {
    const dataUrl = await readFileAsDataUrl(rawFile)
    vs.setMimoCloneAudio(dataUrl, rawFile.name)
  } catch (err) {
    console.error('[VoiceSettings] Failed to read clone audio:', err)
    message.error(t('settings.voice.mimoCloneAudioReadError'))
  }
  return false
}
  ...
  try {
    if (vs.provider.value === 'webspeech') {
      ...
      await speech.openaiPlay('__test__', text, {
        ...
        streamingEnabled: vs.ttsStreamingEnabled.value,
      })
      ...
      if (vs.mimoModel.value === 'mimo-v2.5-tts-voiceclone' && !vs.mimoCloneAudioDataUrl.value) {
        message.warning(t('settings.voice.mimoCloneAudioRequired'))
        return
      }
      await speech.mimoPlay('__test__', text, {
        ...
        cloneAudioDataUrl: vs.mimoCloneAudioDataUrl.value || undefined,
...
<template>
  <div class="voice-settings">
    ...
    <SettingRow
      :label="t('settings.voice.ttsStreaming')"
      :hint="t('settings.voice.ttsStreamingHint')"
    >
      <NSwitch
        :value="vs.ttsStreamingEnabled.value"
        size="small"
        @update:value="vs.setTtsStreamingEnabled"
      />
    </SettingRow>
      ...
      >
        <div class="upload-row">
          <NUpload
            ...
            @before-upload="beforeMimoCloneAudioUpload"
          >
            <NButton size="small">{{ t('settings.voice.mimoCloneAudioUpload') }}</NButton>
          </NUpload>
          <span v-if="vs.mimoCloneAudioName.value" class="audio-file-name">{{ vs.mimoCloneAudioName.value }}</span>
          <NButton
            v-if="vs.mimoCloneAudioDataUrl.value"
            size="small"
            quaternary
            @click="vs.clearMimoCloneAudio"
          >

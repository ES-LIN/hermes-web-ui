...
export const useChatStore = defineStore('chat', () => {
  ...
  async function loadSessions(profile?: string | null, preferredSessionId?: string | null, source?: string | null) {
    isLoadingSessions.value = true
    try {
      const list = await fetchSessions(source || undefined, undefined, profile || undefined)
  ...
  function createSession(options: { profile?: string; model?: string; provider?: string; source?: string } = {}): Session {
  ...
  function newChat(options: { profile?: string; model?: string; provider?: string; source?: string } = {}): Session {
  ...
  async function sendMessage(content: string, attachments?: Attachment[]) {
    ...
    try {
      ...
      const runPayload = {
        ...
        queue_id: userMsg.id,
        source: activeSession.value?.source === 'admin_test' ? 'admin_test' as const : 'cli' as const,
      }

...
import { botLogger, logger } from '../../services/logger'
  ...
  try {
    ...
    if (restart !== false && PLATFORM_SECTIONS.has(section)) {
      try {
        ...
        if (section === 'weixin' || section === 'qqbot') {
          botLogger.info({ profile, platform: section, result: restartResult }, '[config] gateway restarted after bot config update')
        }
      } catch (err) {
        logger.error(err, 'Gateway restart failed')
        if (section === 'weixin' || section === 'qqbot') {
          botLogger.error({ err, profile, platform: section }, '[config] bot gateway restart failed after config update')
        }
        ctx.status = 500
...
export async function updateCredentials(ctx: any) {
  ...
  try {
    ...
    try {
      ...
      if (platform === 'weixin' || platform === 'qqbot') {
        botLogger.info({ profile, platform, result: restartResult }, '[config] gateway restarted after bot credentials update')
      }
    } catch (err) {
      logger.error(err, 'Gateway restart failed')
      if (platform === 'weixin' || platform === 'qqbot') {
        botLogger.error({ err, profile, platform }, '[config] bot gateway restart failed after credentials update')
      }
      ctx.status = 500
      ctx.body = { error: err instanceof Error ? err.message : 'Gateway restart failed' }
      return

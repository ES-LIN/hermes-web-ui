...
function testSessionPrefix(): string {
  const now = new Date()
  return [
    now.getFullYear(),
...
export async function clearAdminTestSessions(ctx: any) {
  const profile = explicitProfileFilter(ctx)
  const sessions = localListSessions(profile, 'admin_test', 5000)
  let deleted = 0
  for (const session of filterByAllowedProfiles(ctx, sessions)) {
    if (localDeleteSession(session.id)) {
      deleteUsage(session.id)
      deleted += 1
    }
  }
...
export async function promoteAdminTestSession(ctx: any) {
  const id = String(ctx.params.id || '')
  const target = localGetSessionDetail(id)
  if (!target) {
    ctx.status = 404
    ctx.body = { error: 'Session not found' }
    return
  }
  if (denySessionAccess(ctx, target)) return
  if (target.source !== 'admin_test') {
    ctx.status = 400
    ctx.body = { error: 'Only admin test sessions can be promoted' }
    return
  }

  const newId = `${testSessionPrefix()}_${Math.random().toString(16).slice(2, 8)}`
  localCreateSession({
    id: newId,
    profile: target.profile || requestedProfile(ctx) || 'default',
    source: 'cli',
    model: target.model,
    provider: target.provider,
    title: target.title || target.preview || '',
    workspace: target.workspace || undefined,
  })
  localAddMessages(target.messages.map(message => ({
    session_id: newId,
    role: message.role,
  ...
  localUpdateSessionStats(newId)
  ctx.body = { success: true, session: localGetSessionDetail(newId) }
}
...
export async function listHermesSessions(ctx: any) {
  ...
  ctx.body = { sessions: filterPendingDeletedSessions(filterByAllowedProfiles(ctx, allSessions).filter(s => s.source !== 'api_server' && s.source !== 'admin_test')) }
}
...
export async function getHermesSession(ctx: any) {
  ...
  if (localSession && localSession.source !== 'api_server' && localSession.source !== 'admin_test' && (!profile || localSessionProfile === profile)) {
  ...
  try {
    ...
    if (session && session.source !== 'api_server' && session.source !== 'admin_test') {
  ...
  if (session.source === 'api_server' || session.source === 'admin_test') {
    ctx.status = 404
    ctx.body = { error: 'Session not found' }
    return
  }

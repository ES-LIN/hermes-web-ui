...
export async function save(ctx: any) {
  ...
  try {
    ...
    botLogger.info({ profile, platform: 'weixin', account_id, has_base_url: Boolean(base_url) }, '[weixin] credentials saved')
    const restartResult = await restartGatewayForProfile(profile)
    botLogger.info({ profile, platform: 'weixin', result: restartResult }, '[weixin] gateway restarted after credentials save')
    ctx.body = { success: true }
  } catch (err: any) {
    botLogger.error({ err }, '[weixin] failed to save credentials or restart gateway')

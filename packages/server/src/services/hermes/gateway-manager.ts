...
import { createWriteStream, readFileSync, existsSync, readdirSync, unlinkSync, mkdirSync } from 'fs'
...
export class GatewayManager {
  ...
  private async startResolved(name: string, endpoint: ResolvedGatewayEndpoint): Promise<GatewayStatus> {
    ...
    return new Promise((resolve, reject) => {
      ...
      botLogger.info({ profile: name, pid, port, host, detached: detachGateway }, '[gateway] starting managed gateway')

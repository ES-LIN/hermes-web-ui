...
export async function handleBridgeRun(
  ...
  if (shouldPersistUserMessage) {
    ...
    if (!getSession(session_id)) {
      ...
      createSession({ id: session_id, profile, source: data.source === 'admin_test' ? 'admin_test' : 'cli', model: resolvedModel, provider: resolvedProvider, title: preview })
    }
...
async function maybeEnqueueGoalContinuation(args: {
  ...
  const next: QueuedRun = {
    ...
    profile: args.profile,
    source: args.state.source === 'admin_test' ? 'admin_test' : 'cli',

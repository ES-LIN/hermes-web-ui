...
const routerCurrentRoute = { value: { name: 'admin.logs' } }
...
describe('session search modal', () => {
  beforeEach(() => {
    ...
    routerCurrentRoute.value = { name: 'admin.logs' }
  })
  ...
  it('searches by content and opens the matched session', async () => {
    ...
    expect(apiMocks.routerPushMock).toHaveBeenCalledWith({ name: 'admin.chat' })
  })
})

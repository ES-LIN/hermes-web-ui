...
test('sidebar navigation exposes native links', async ({ page }) => {
  ...
  await expect(models).toHaveAttribute('href', '#/admin/models')

  const history = page.locator('aside.sidebar').getByRole('link', { name: /^History$/ })
  await expect(history).toHaveAttribute('href', '#/admin/history')
})
...
test('session rows expose native session links', async ({ page }) => {
  ...
  await expect(sessionLink).toHaveAttribute('href', '#/admin/session/session-native-1')

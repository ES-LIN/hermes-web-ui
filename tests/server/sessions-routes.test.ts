...
const clearAdminTestSessionsMock = vi.fn(async (ctx: any) => { ctx.body = { success: true, deleted: 1 } })
const promoteAdminTestSessionMock = vi.fn(async (ctx: any) => { ctx.body = { success: true, session: { id: 'promoted-1' } } })
...
vi.mock('../../packages/server/src/controllers/hermes/sessions', () => ({
  ...
  clearAdminTestSessions: clearAdminTestSessionsMock,
  promoteAdminTestSession: promoteAdminTestSessionMock,
}))
...
describe('session routes', () => {
  beforeEach(() => {
    ...
    clearAdminTestSessionsMock.mockClear()
    promoteAdminTestSessionMock.mockClear()
  })
  ...
  it('registers conversations, session list, and search routes', async () => {
    ...
    expect(paths).toEqual(expect.arrayContaining([
      ...
      '/api/hermes/sessions/admin-test',
      '/api/hermes/sessions/admin-test/:id/promote',
  ...
  it('delegates admin test session management routes', async () => {
    const { sessionRoutes } = await import('../../packages/server/src/routes/hermes/sessions')
    const clearLayer = sessionRoutes.stack.find((entry: any) => entry.path === '/api/hermes/sessions/admin-test')
    const promoteLayer = sessionRoutes.stack.find((entry: any) => entry.path === '/api/hermes/sessions/admin-test/:id/promote')

    const clearCtx: any = { params: {}, query: {}, body: null }
    await clearLayer.stack[0](clearCtx)
    expect(clearAdminTestSessionsMock).toHaveBeenCalledWith(clearCtx)
    expect(clearCtx.body).toEqual({ success: true, deleted: 1 })

    const promoteCtx: any = { params: { id: 'admin-test-1' }, query: {}, body: null }
    await promoteLayer.stack[0](promoteCtx)
    expect(promoteAdminTestSessionMock).toHaveBeenCalledWith(promoteCtx)
    expect(promoteCtx.body).toEqual({ success: true, session: { id: 'promoted-1' } })
  })
})
